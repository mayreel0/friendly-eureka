...
test('recording import is bounded, revision guarded, durable and draft-only', async () => {
  ...
  try {
    ...
    const replaced = await (await post('pilot-route-import', fixture, active.revision)).json();
    assert.equal(replaced.recording.routeVersion, imported.recording.routeVersion + 1);
